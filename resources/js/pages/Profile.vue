...
interface Props {
    ...
    repoCountsByProvider: Record<string, number>;
}
...
const providerIcons: Record<string, { name: string; svg: string }> = {
    github: {
        name: 'GitHub',
        svg: `<svg fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 0C4.477 0 0 4.484 0 10.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0110 4.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.203 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.942.359.31.678.921.678 1.856 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0020 10.017C20 4.484 15.522 0 10 0z" clip-rule="evenodd"></path></svg>`,
    },
    gitlab: {
        name: 'GitLab',
...
const providerDisplay = computed(() => {
    ...
    const merged: Record<string, { name: string; icon: string; count: number }> = {};

    props.connectedProviders.forEach((provider) => {
        const key = provider === 'jira' ? 'bitbucket' : provider;

        if (!gitProviders.includes(key)) return;

        if (!merged[key]) {
            merged[key] = {
                name: providerIcons[key as keyof typeof providerIcons].name,
                icon: providerIcons[key as keyof typeof providerIcons].svg,
                count: 0,
            };
        }
        merged[key].count += props.repoCountsByProvider[provider] || 0;
    });
...
const syncProviderSkills = (provider: 'github' | 'gitlab' | 'bitbucket') => {
    if (syncingProvider.value) return;

    const route = providerRoutes[provider];
    if (!route) {
        showError('Unsupported provider');
        return;
    }
    ...
    router.post(
        ...
        {},
        {
            ...
            onSuccess: () => {
                ...
                router.reload({ only: ['user', 'repoCountsByProvider', 'connectedProviders'] });
            },
            onError: () => {
            ...
            <div class="mx-auto max-w-4xl space-y-6 px-4 sm:px-6 lg:px-8">
                ...
                <Card v-if="providerDisplay && providerDisplay.length > 0" class="border-gray-200 bg-white/40 dark:border-white/10 dark:bg-white/5">
                    <CardHeader>
                        ...
                        <CardDescription>
                            {{ isOwner ? 'Your connected accounts and repositories' : `${user.name}'s connected accounts and repositories` }}
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div class="flex flex-wrap gap-3">
                            <div
                                v-for="provider in providerDisplay"
                                :key="provider.name"
                                class="flex items-center gap-2 rounded-lg border border-gray-200 bg-white/60 px-4 py-2 dark:border-white/10 dark:bg-white/5"
                            >
                                <div class="h-5 w-5" v-html="provider.icon"></div>
                                <span class="font-medium">{{ provider.name }}</span>
                                <Badge variant="secondary" class="ml-1">{{ provider.count }}</Badge>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <Card v-if="user.skills && user.skills.length > 0" class="border-gray-200 bg-white/40 dark:border-white/10 dark:bg-white/5">
                        <CardHeader>
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between sm:gap-4 px-4">
                                ...
                                <ProviderSyncButtons
                                    v-if="isOwner"
                                    :syncing-provider="syncingProvider"
                        ...
                        <CardHeader>
                            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                                    ...
                                    <CardDescription>Sync your skills from repositories!</CardDescription>
                                </div>
                        ...
                        <CardContent>
                            <div class="py-8 text-center text-muted-foreground">
                                ...
                                <p>
                                    {{
                                        isOwner
                                            ? 'No skills earned yet. Sync from GitHub, GitLab or Bitbucket, or complete your first bounty to start earning XP!'
