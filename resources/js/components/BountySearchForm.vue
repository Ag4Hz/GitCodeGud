...
import { AlertCircle, ExternalLink, Info, Loader2, MessageCircle, Search, X } from 'lucide-vue-next';
...
const isBitbucketRepo = computed(() => selectedRepo.value?.provider === 'bitbucket');
const hasJiraLinked   = computed(() => props.connectedProviders?.includes('jira') ?? false);
...
const gitProviders = computed(() =>
    (props.connectedProviders ?? []).filter((p) => p !== 'jira'),
);
...
const debouncedSearchRepositories = () => {
    if (repositorySearchTimeout) clearTimeout(repositorySearchTimeout);
    repositorySearchTimeout = setTimeout(searchRepositories, 500);
};

const handleRepositoryFocus = () => {
    if (props.repositories.length > 0) showRepositoryDropdown.value = true;
};
...
const searchRepositories = () => {
    ...
    router.visit(route('bounty.search-repositories'), {
        method: 'get',
        data: { query: repositorySearchQuery.value },
        preserveState: true,
        preserveScroll: true,
        replace: true,
        onFinish: () => {
...
const searchIssues = () => {
    if (!selectedRepo.value || isBitbucketRepo.value) return;
...
const selectIssue = (issue: Issue) => {
    ...
    if (!props.form.description && issue.body) {
        ...
        updateFormField('description', `Related to issue: ${issue.url}\n\n${truncatedBody}`);
    }
};

const handleJiraUrlInput = () => {
    updateFormField('jira_issue_url', jiraIssueUrl.value);
};
...
const clearIssue = () => {
    ...
    updateFormField('jira_issue_url', '');
    showIssueDropdown.value = false;
};
...
watch(selectedProviderFilter, () => {
    if (repositorySearchQuery.value.trim()) debouncedSearchRepositories();
});

watch(
    () => props.form.errors,
    (errors) => {
        if (errors.repository_full_name) showRepositoryDropdown.value = false;
        if (errors.issue_number) showIssueDropdown.value = false;
    },
    { deep: true },
);
...
<template>
    <div class="space-y-6">
        ...
        <div class="space-y-2">
            ...
            <!-- Provider Filter (Git providers only, no Jira) -->
            <div v-if="gitProviders.length > 1" class="flex gap-2">
                <Button
                    type="button"
                    variant="outline"
                    size="sm"
                        ...
                        <p class="text-sm text-muted-foreground">
                            {{
                                selectedProviderFilter !== 'all'
                                    ? `No ${getProviderName(selectedProviderFilter)} repositories found`
            ...
            <div v-if="selectedRepo" class="rounded-2xl border border-gray-300 p-3">
                <div class="flex items-center justify-between">
                    ...
                    <Button variant="ghost" size="sm" @click="clearRepository">
                        <X class="h-4 w-4" />
                    </Button>
                </div>
            </div>
        ...
        <div v-if="selectedRepo" class="space-y-2">
            ...
            <template v-if="isBitbucketRepo">
                <Label>Jira Issue URL *</Label>
                <p class="text-sm text-muted-foreground">
                    Bitbucket repositories use Jira for issue tracking. Paste the Jira issue URL below.
                </p>
                <div
                    v-if="!hasJiraLinked"
                ...
                >
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h4 class="font-medium text-amber-800 dark:text-amber-200">Jira account not connected</h4>
                            <p class="text-sm text-amber-700 dark:text-amber-300">
                                ...
                                in Account Settings to validate Jira issues.
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="relative">
                    <Input
                        v-model="jiraIssueUrl"
                        placeholder="https://yourworkspace.atlassian.net/browse/PROJ-123"
                        :class="[form.errors.jira_issue_url && 'border-red-500 focus-visible:ring-red-500']"
                        :disabled="!hasJiraLinked"
                        @input="handleJiraUrlInput"
                    />
                </div>
                <InputError :message="form.errors.jira_issue_url" />

                <!-- Issues Dropdown -->

                <div
                    v-if="jiraIssueUrl && !form.errors.jira_issue_url"
                    class="flex items-center gap-2 text-sm text-muted-foreground"
                >
                    <ExternalLink class="h-3.5 w-3.5" />
                    <a :href="jiraIssueUrl" target="_blank" rel="noopener" class="truncate hover:underline">
                        {{ jiraIssueUrl }}
                    </a>
                </div>
            </template>
            ...
            <template v-else>
                    ...
                    >
                        ...
                        <div v-else class="max-h-60 overflow-y-auto">
                            ...
                            >
                                ...
                                >
                                    <div class="mt-1 ml-3 flex flex-shrink-0 items-center gap-2">
                ...
                <div v-if="selectedIssue" class="rounded-2xl border border-gray-300 p-3">
                    <div class="flex items-start justify-between">
                        ...
                        <Button variant="ghost" size="sm" class="ml-2 flex-shrink-0" @click="clearIssue">
                            <X class="h-4 w-4" />
                        </Button>
                    </div>
                </div>
                ...
                <div
                    v-if="!selectedIssue && !issueLoading"
